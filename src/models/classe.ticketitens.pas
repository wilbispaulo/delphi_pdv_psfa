...
type
  ...
  public
    ...
    function fncTotalizaTicket: Currency;
    function fncGravaItem: Boolean;
    function fncAtualizaDesc: Boolean;
...
function TTicketItens.fncAtualizaDesc: Boolean;
var
  lsCodProd: string;
...
begin
  ...
  try
    ...
    QryConsulta.ParamByName('pNumticket').AsString := Self.numticket;
    QryConsulta.ParamByName('pCodGrupo').AsString := Self.codgrupo;
    QryConsulta.Open;
...
begin
  try
    ...
    prcSQLInit;
    try
      QryConsulta.SQL.Add('INSERT INTO ticketitens ');
      QryConsulta.SQL.Add('(numticket, codgrupo, codprod, descr, qtd, und, vlun, ');
      QryConsulta.SQL.Add('descun, descgrp, especial, descresp, pedido_em, ');
      QryConsulta.SQL.Add('entregue, entregue_em, cancelado, cancelado_em) ');
      QryConsulta.SQL.Add('VALUES (:pnumticket, :pcodgrupo, :pcodprod, :pdecr, ');
      QryConsulta.SQL.Add(':pqtd, :pund, :pvlun, :pdescun, :pdescgrp, :pespecial, ');
      QryConsulta.SQL.Add(':pdescresp, :ppedido_em, :pentregue, :pentregue_em, ');
      QryConsulta.SQL.Add(':pcancelado, :pcancelado_em)');
      QryConsulta.ParamByName('pnumticket').AsString      := Self.numticket;
      QryConsulta.ParamByName('pcodgrupo').AsString       := Self.codgrupo;
      QryConsulta.ParamByName('pcodprod').AsString        := Self.codprod;
      QryConsulta.ParamByName('pdescr').AsString          := Self.descr;
      QryConsulta.ParamByName('pqtd').AsInteger           := Self.qtd;
      QryConsulta.ParamByName('pund').AsString            := Self.und;
      QryConsulta.ParamByName('pvlun').AsCurrency         := Self.vlun;
      QryConsulta.ParamByName('pdescun').AsCurrency       := Self.descun;
      QryConsulta.ParamByName('pdescgrp').AsCurrency      := Self.descgrp;
      QryConsulta.ParamByName('pespecial').AsString       := Self.especial;
      QryConsulta.ParamByName('pdescresp').AsString       := Self.descresp;
      QryConsulta.ParamByName('ppedido_em').AsDateTime    := Now();
      QryConsulta.ParamByName('pentregue').AsString       := Self.entregue;
      QryConsulta.ParamByName('pentregue_em').AsDateTime  := Self.entregue_em;
      QryConsulta.ParamByName('pcancelado').AsString      := Self.cancelado;
      QryConsulta.ParamByName('pcancelado_em').AsDateTime := Self.cancelado_em;
      QryConsulta.ExecSQL;
...
function TTicketItens.fncTotalizaTicket: Currency;
var
