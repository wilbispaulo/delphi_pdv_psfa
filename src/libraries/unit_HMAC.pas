...
type
  ...
  public
    ...
    class function ValidatePassword(aKey, aUser, aValue, aHmacHash: string): Boolean;
...
begin
  ...
  if (iID > 0) and (iID <= 999) then
  begin

    sSubKey := IntToHex(int16(strtoint('0x' + aKey.Substring(28)) + iID));
    aKey := aKey.Substring(0,28) + sSubKey.ToLower;
...
class function THMAC<T>.ValidatePassword(aKey, aUser, aValue, aHmacHash: string): Boolean;
var
  hmac: T;
  valueHmac: string;
  begin
    valueHmac :=  self.HashValueBase64(aKey + aUser.ToLower, aValue);
